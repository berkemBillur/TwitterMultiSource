...
################################################################################################################
    ...
    def process_searched_targets(tweet):
        ...
        'retweet_count': tweet.retweeted_count, 'favourite_count':tweet.favorite_count}
...
####################################################################################
    ...
    def stage1_extraction(self):
        ...
        for tweet_id, data in list(targets_without_friends.copy().items()):
            ...
            if count_users_pulled %9 == 0 : # can only make 15 requests per API. So after 15 requests, move onto next API
